...
const CalendarPage = () => {
  ...
  const [selected, setSelected] = useState(null); // { day, month, apps[] }
  ...
  const appsByDate = useMemo(() => {
    ...
    applications.forEach((app) => {
      const applied = new Date(app.appliedDate || app.createdAt);
      const appliedKey = `${applied.getFullYear()}-${applied.getMonth()}-${applied.getDate()}`;
      if (!map[appliedKey]) map[appliedKey] = [];
      map[appliedKey].push(app);

      if (app.interviewDate) {
        const interview = new Date(app.interviewDate);
        const interviewKey = `${interview.getFullYear()}-${interview.getMonth()}-${interview.getDate()}`;
        if (!map[interviewKey]) map[interviewKey] = [];
        if (!map[interviewKey].includes(app)) map[interviewKey].push(app);
      }
    });
    return map;
  }, [applications]);
  ...
  const thisMonthApps = useMemo(() => {
    return applications
      .map((app) => {
        ...
        const applied = new Date(app.appliedDate || app.createdAt);
        if (applied.getFullYear() === year && applied.getMonth() === month) {
          dates.push(applied);
        }
        ...
        if (app.interviewDate) {
          const interview = new Date(app.interviewDate);
          if (
            interview.getFullYear() === year &&
            interview.getMonth() === month
          ) {
            dates.push(interview);
          }
        }
        ...
        return {
          app,
          sortKey: Math.min(...dates.map((d) => d.getTime())),
        };
      })
      .filter(Boolean)
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/5 bg-[#18181B] p-4 sm:p-6 shadow-xl">
          ...
          <div className="grid grid-cols-7 mb-2">
            {DAYS.map((d) => (
              <div
                key={d}
                className="text-center text-[10px] sm:text-xs font-semibold text-zinc-500 py-1"
              >
                {d}
              </div>
            ))}
          ...
          {selected && selected.apps.length > 0 && (
            <div className="mt-5 pt-5 border-t border-white/5">
              <p className="text-sm font-semibold text-zinc-300 mb-3">
                {MONTHS[selected.month]} {selected.day} — {selected.apps.length}{" "}
                application{selected.apps.length !== 1 ? "s" : ""}
              </p>
              <div className="space-y-2">
                {selected.apps.map((app) => (
                  <div
                    key={app._id}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-zinc-800/60"
                  >
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 ${STATUS_COLOR[app.status] || "bg-zinc-500"}`}
                    />
          ...
          <div className="rounded-2xl border border-white/5 bg-[#18181B] p-5 shadow-xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-xs font-semibold text-zinc-500">
                {thisMonthApps.length} app
                {thisMonthApps.length !== 1 ? "s" : ""}
              </span>
            </div>
            ...
            {thisMonthApps.length > 0 ? (
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                {thisMonthApps.map((app) => (
                  <div
                    key={app._id}
                    className="flex items-start gap-3 px-3 py-2.5 rounded-xl bg-zinc-800/50 hover:bg-zinc-800 transition-colors"
                  >
                    <div
                      className={`w-1.5 h-full min-h-[32px] rounded-full shrink-0 mt-0.5 ${STATUS_COLOR[app.status] || "bg-zinc-500"}`}
                    />
